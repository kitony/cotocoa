...
//static int *special_buffer_test;
...
static int requester_buffer_init(void *address, int buffersize_tmp, int dimensionsize_tmp, int *location_start_tmp, int *location_end_tmp, int type)
{
    //printf("requester:init start\n");
    int datasize_tmp = 1;
    int color = 0;
    int i;
    buffersize = buffersize_tmp;
    //datasize = (int *)malloc(dimensionsize_tmp * sizeof(int));
    for (i = 0; i < dimensionsize_tmp;i++)
    {
        datasize_tmp = datasize_tmp * (location_end_tmp[i] - location_start_tmp[i]);
    }
    //printf("requester:rank is%d,datasize is %d\n", world_myrank, datasize_tmp);
    ...
    //printf("requester:new signal2\n");
    //MPI_Win_create(special_buffer, buffersize * datasize_tmp * sizeof(int), sizeof(int), MPI_INFO_NULL, MPI_COMM_WORLD, &win_test[0]);

    printf("requester:init start\n");

    switch (type) {
        case DAT_INT:
        MPI_Win_create(address, buffersize * datasize_tmp * sizeof(int), sizeof(int), MPI_INFO_NULL, MPI_COMM_WORLD, &win_data);
        break;
        case DAT_REAL4:
        MPI_Win_create(address, buffersize * datasize_tmp * sizeof(float), sizeof(float), MPI_INFO_NULL, MPI_COMM_WORLD, &win_data);
        break;
        case DAT_REAL8:
        MPI_Win_create(address, buffersize * datasize_tmp * sizeof(double), sizeof(double), MPI_INFO_NULL, MPI_COMM_WORLD, &win_data);
        break;
    }

    printf("requester:signal inp\n");
    ...
    //MPI_Win_lock_all(MPI_MODE_NOCHECK,win_test[0]);
    ...
    printf("requester:init end\n");
}

int CTCAR_buffer_init_int(int *special_buffer, int buffersize_tmp, int dimensionsize_tmp, int *location_start_tmp, int *location_end_tmp)
{
    //printf("requester:CTCAR_buffer_init_int is start\n");
    if (myrole != ROLE_REQ)
    {
        fprintf(stderr, "%d : CTCAR_buffer_init_int() : ERROR : wrong role %d\n", world_myrank, myrole);
        return 0;
    }
    requester_buffer_init((void *)special_buffer, buffersize_tmp, dimensionsize_tmp, location_start_tmp, location_end_tmp, DAT_INT);
}

int CTCAR_buffer_init_real4(float *special_buffer, int buffersize_tmp, int dimensionsize_tmp, int *location_start_tmp, int *location_end_tmp)
{
    //printf("requester:CTCAR_buffer_init_int is start\n");
    if (myrole != ROLE_REQ)
    {
    ...
    requester_buffer_init((void *)special_buffer, buffersize_tmp, dimensionsize_tmp, location_start_tmp, location_end_tmp, DAT_REAL4);
}

int CTCAR_buffer_init_real8(double *special_buffer, int buffersize_tmp, int dimensionsize_tmp, int *location_start_tmp, int *location_end_tmp)
{
    //printf("requester:CTCAR_buffer_init_int is start\n");
    if (myrole != ROLE_REQ)
    {
    ...
    requester_buffer_init((void *)special_buffer, buffersize_tmp, dimensionsize_tmp, location_start_tmp, location_end_tmp, DAT_REAL8);
}
...
static int coupler_buffer_init()
{
    int color = 2;
    MPI_Comm_split(MPI_COMM_WORLD, color, world_myrank, &sub_comm_buffer);
    //MPI_Win_create(NULL, 0, sizeof(int), MPI_INFO_NULL, MPI_COMM_WORLD, &win_test[0]);
...
static int worker_buffer_init(int buffersize_tmp, int loop_number_tmp, int *recv_location_start, int *recv_location_end, int type)
{
    printf("wroker:init start\n");
    int i,j;
    int color = 1;
    buffersize = buffersize_tmp;
    ...
    if (subrank_table[world_myrank] == 0)
    {
        ...
        {
            MPI_Recv(location_start + requesterid_table[i] * dimensionsize, dimensionsize, MPI_INT, requesterid_table[i], 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            MPI_Recv(location_end + requesterid_table[i] * dimensionsize, dimensionsize, MPI_INT, requesterid_table[i], 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
        //datasize = (int *)malloc(numrequesters * sizeof(int));
    ...
    for (j = 0;j < numrequesters;j++)
    {
        if (calculated[j] > 0)
        {
            for (i = 0;i < dimensionsize;i++)
            {
                ...
                {
                    real_start[j * dimensionsize + i] = location_start[j * dimensionsize + i] - location_start[j * dimensionsize + i];
                }
    ...
    receiver_start  = (int *)malloc(dimensionsize * numrequesters * sizeof(int));

    printf("wroker:init 3\n");
    ...
    for (j = 0;j < numrequesters;j++)
    {
        if (calculated[j] > 0)
        {
            for (i = 0;i < dimensionsize;i++)
            {
                array_count[dimensionsize - 1 - i] = location_end[j * dimensionsize + i] - location_start[j * dimensionsize + i];
                subarray_count[dimensionsize - 1 - i] = real_end[j * dimensionsize + i] - real_start[j * dimensionsize + i];

                subarray_coordinates[dimensionsize - 1 - i] = real_start[j * dimensionsize + i];
                receiver_array_count[dimensionsize - 1 - i] = recv_location_end[i] - recv_location_start[i];
                receiver_subarray_count[dimensionsize - 1 - i] = subarray_count[dimensionsize - 1 - i];

                receiver_subarray_coordinates[dimensionsize - 1 - i] = 0;
                receiver_start[j * dimensionsize + i] = real_end[j * dimensionsize + i] + location_start[j * dimensionsize + i] - recv_location_start[i] - subarray_count[dimensionsize - 1 - i];
            }
            switch (type) {
                case DAT_INT:
    ...
    printf("wroker:init end\n");
}

int CTCAW_buffer_init_int(int buffersize_tmp, int loop_number_tmp, int *recv_location_start, int *recv_location_end)
{
    if (myrole != ROLE_WRK)
    {
    ...
    worker_buffer_init(buffersize_tmp, loop_number_tmp, recv_location_start, recv_location_end, DAT_INT);
}

int CTCAW_buffer_init_real4(int buffersize_tmp, int loop_number_tmp, int *recv_location_start, int *recv_location_end)
{
    if (myrole != ROLE_WRK)
    {
        fprintf(stderr, "%d : CTCAW_buffer_init_int() : ERROR : wrong role %d\n", world_myrank, myrole);
    ...
    worker_buffer_init(buffersize_tmp, loop_number_tmp, recv_location_start, recv_location_end, DAT_REAL4);
}

int CTCAW_buffer_init_real8(int buffersize_tmp, int loop_number_tmp, int *recv_location_start, int *recv_location_end)
{
    if (myrole != ROLE_WRK)
    {
        fprintf(stderr, "%d : CTCAW_buffer_init_int() : ERROR : wrong role %d\n", world_myrank, myrole);
    ...
    worker_buffer_init(buffersize_tmp, loop_number_tmp, recv_location_start, recv_location_end, DAT_REAL8);
}

static int requester_buffer_load_data(void *address, void *loaded_data, int type)
{
    if (subrank_table[world_myrank] == 0)
    {
        count_requester++;
    ...
    int i, j;
    int mod_number;
    //printf("world rank is %d, sub rank is %d\n", world_myrank , tmp1);
    MPI_Bcast((void *)&count_requester, 1, MPI_INT, 0, sub_comm_buffer);
    //printf("requester:bcast1 end\n");
    mod_number = count_requester % buffersize;
    //if the data in buffer is full, then requester should wait for worker
    if (subrank_table[world_myrank] == 0)
    {
        while (count_requester >= count_worker + buffersize)
        {
            /*
            tmp = *count_requester;
            tmp2 = *count_worker + buffersize;
            if (tmp < tmp2)
            {
                break;
            }
    ...
    //printf("requester:bcast2 start\n");
    MPI_Bcast((void *)&count_worker, 1, MPI_INT, 0, sub_comm_buffer);
    //printf("requester:bcast2 end\n");
    //copy data to the exact location in buffer
    switch (type) {
        case DAT_INT:
        memcpy((int *)address + datasize[subrank_table[world_myrank]] * mod_number, (int *)loaded_data, datasize[subrank_table[world_myrank]] * sizeof(int));
        break;
        case DAT_REAL4:
        memcpy((float *)address + datasize[subrank_table[world_myrank]] * mod_number, (float *)loaded_data, datasize[subrank_table[world_myrank]] * sizeof(float));
        break;
        case DAT_REAL8:
        memcpy((double *)address + datasize[subrank_table[world_myrank]] * mod_number, (double *)loaded_data, datasize[subrank_table[world_myrank]] * sizeof(double));
        break;
    }
    //memcpy(special_buffer + datasize[subrank_table[world_myrank]] * mod_number, loaded_data, datasize[subrank_table[world_myrank]] * sizeof(int));

    if (subrank_table[world_myrank] == 0)
    {
        printf("requester:count_requester is %d, count_worker is %d\n", count_requester, count_worker);
        //printf("headworker is %d\n", head_worker);
        tmp = count_requester;
        //MPI_Put((void *)&count_requester, 1, MPI_INT, head_worker, 0, 1, MPI_INT, win_count_requester);
        MPI_Put(&tmp, 1, MPI_INT, head_worker, 0, 1, MPI_INT, win_count_requester);
        //MPI_Win_flush(head_worker, win_count_requester);
    }
    //printf("requester:put end\n");
}

int CTCAR_load_data_int(int *special_buffer, int *loaded_data)
{
    if (myrole != ROLE_REQ)
    {
        fprintf(stderr, "%d : CTCAR_buffer_init_int() : ERROR : wrong role %d\n", world_myrank, myrole);
    ...
    requester_buffer_load_data((void *)special_buffer, (void *)loaded_data, DAT_INT);
}

int CTCAR_load_data_real4(float *special_buffer, float *loaded_data)
{
    if (myrole != ROLE_REQ)
    {
        fprintf(stderr, "%d : CTCAR_buffer_init_int() : ERROR : wrong role %d\n", world_myrank, myrole);
    ...
    requester_buffer_load_data((void *)special_buffer, (void *)loaded_data, DAT_REAL4);
}

int CTCAR_load_data_real8(double *special_buffer, double *loaded_data)
{
    if (myrole != ROLE_REQ)
    {
        fprintf(stderr, "%d : CTCAR_buffer_init_int() : ERROR : wrong role %d\n", world_myrank, myrole);
    ...
    requester_buffer_load_data((void *)special_buffer, (void *)loaded_data, DAT_REAL8);
}

static int test = 0;

int CTCAW_get_data_int(int *receiver_buffer)
{
    if (myrole != ROLE_WRK)
    {
        fprintf(stderr, "%d : CTCAW_get_data() : ERROR : wrong role %d\n", world_myrank, myrole);
        return 0;
    }
        if (count_worker == loop_number - 1)
    {
        printf("worker:the work is finished!\n");
        return 2;
    }
    int tmp;
    int i ,j;
    ...
    if (subrank_table[world_myrank] == 0)
    {
        if (count_requester > count_worker)
        {
            if (test == 0)
            {
                printf("worker:good! %d %d\n", count_requester, count_worker);
                test++;
            }
            count_worker++;
            //printf("worker:signal important s\n");
            //MPI_Bcast((void *)&count_requester, 1, MPI_INT, 0, sub_comm_buffer);
            ...
            for (j = 0;j < numrequesters;j++)
            {
                if (calculated[j] == 1)
                {
                    receiver_tmp = receiver_buffer;
                    for (i = 0;i < dimensionsize;i++)
                    {
                        receiver_tmp += dimension_max[dimensionsize - i - 1] * receiver_start[j * dimensionsize + i];
                    }
                    //printf("worker:signal important ss\n");
                    receiver_tmp2 = (void *)receiver_tmp;
                    MPI_Get(receiver_tmp2, 1, receiver_subarray_type[j], j, mod_number * datasize[j], 1, subarray_type[j], win_data);
                    printf("worker:signal important sss\n");
                    MPI_Win_flush(j, win_data);
                    printf("worker:signal important ssss\n");
                }
            }
            printf("worker%d:count_requester is %d, count_worker is %d\n", world_myrank, count_requester, count_worker);
            tmp = count_worker;
            MPI_Put(&tmp, 1, MPI_INT, head_requester, 0, 1, MPI_INT, win_count_worker);
            //MPI_Win_flush(head_requester, win_count_worker);
            printf("worker:happy\n");
            return 1;
        }
        else return 0;
...
/*
int CTCAW_get_data_real4(float *receiver_buffer, volatile int *count_requester, volatile int *count_worker)
{
    if (myrole != ROLE_WRK)
    {
        fprintf(stderr, "%d : CTCAW_get_data() : ERROR : wrong role %d\n", world_myrank, myrole);
...
int CTCAW_get_data_real8(double *receiver_buffer, volatile int *count_requester, volatile int *count_worker)
{
    if (myrole != ROLE_WRK)
    {
        fprintf(stderr, "%d : CTCAW_get_data() : ERROR : wrong role %d\n", world_myrank, myrole);
