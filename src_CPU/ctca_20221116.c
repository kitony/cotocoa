...
int CTCAW_get_timestep()
{
    return current_timestep + tmp_get_address - residue + 1;
}

int CTCAW_get_overwrite_flag()
{
...
int* CTCAW_get_address(int *receiver_address)
{
...
int worker_buffer_get_data_nosuspend(int *receiver_address)
{
    ...
    if (subrank_table[world_myrank] == 0)
    {
        ...
        printf("worker%d: show current_timestep %d, count_worker is %d\n", world_myrank, current_timestep, count_worker);
...
int CTCAW_buffer_get_data_int(int *receiver_address)
{
    ...
    {
        return worker_buffer_get_data_nosuspend(receiver_address);
    }
}
