...
int worker_buffer_get_data_nosuspend(int *receiver_address)
{
    if (residue > 0)
    {
        residue--;
    ...
    if (subrank_table[world_myrank] == 0)
    {
        ...
        {
            count_worker++;
            while ((count_worker != tmp_count_requester) && ((tmp_count_requester - count_worker) % buffersize == 0) && (tmp_count_requester != loop_number - 1))
            {
                MPI_Win_flush(head_requester, win_count_requester);
            }
    ...
    else 
    {
        ...
        else 
        {
            ...
            for (j = 0;j < numrequesters;j++)
            {
                if (calculated[j] == 1)
                {
                    MPI_Get(receiver_address + receiver_size * (mod_bound - count_worker - 1), count_requester - mod_bound + 1, receiver_subarray_type[j], requesterid_table[j], 0, count_requester - mod_bound + 1, subarray_type[j], win_data);
                }
            }
        }
