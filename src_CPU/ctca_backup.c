...
static int worker_buffer_get_data(void *receiver_address)
{
    ...
    if (subrank_table[world_myrank] == 0)
    {
        ...
        if (count_requester > count_worker)
        {
            ...
            for (j = 0;j < numrequesters;j++)
            {
                if (calculated[j] == 1)
                {
                    MPI_Get(receiver_address, 1, receiver_subarray_type[j], requesterid_table[j], mod_number * datasize[j], 1, subarray_type[j], win_data);
                }
            }
            ...
            MPI_Put((void *)&count_worker, 1, MPI_INT, head_requester, 0, 1, MPI_INT, win_count_worker);
            return 1;
        }
        else return 0;
    }
