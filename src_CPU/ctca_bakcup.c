    ...
    for (j = 0;j < numrequesters;j++)
    {
        if (calculated[j] > 0)
        {
            for (i = 0;i < dimensionsize;i++)
            {
                ...
                receiver_subarray_coordinates[i] = 0;
                receiver_start[j * dimensionsize + i] = real_end[j * dimensionsize + i] + location_start[j * dimensionsize + i] - recv_location_start[i] - subarray_count[i];
            }
            switch (type) {
                case DAT_INT:
...
static int requester_buffer_load_data(void *address, void *loaded_data, int type)
{
    ...
    if (subrank_table[world_myrank] == 0)
    {
        while (count_requester >= count_worker + buffersize)
        {
            ...
            MPI_Win_flush(head_worker, win_count_worker);
        }
    }
...
int CTCAW_get_data_int(int *receiver_buffer)
{
    if (myrole != ROLE_WRK)
    {
        fprintf(stderr, "%d : CTCAW_get_data_int() : ERROR : wrong role %d\n", world_myrank, myrole);
        return 0;
    }
    if (count_worker == loop_number - 1)
    {
        //return 2 means the whole work is over
        return 2;
    }
    int tmp;
    int i ,j;
        ...
        if (count_requester > count_worker)
        {
            ...
            for (j = 0;j < numrequesters;j++)
            {
                if (calculated[j] == 1)
                {
                    ...
                    for (i = 0;i < dimensionsize;i++)
                    {
                        receiver_tmp += dimension_max[i] * receiver_start[j * dimensionsize + i];
                    }
                    receiver_tmp2 = (void *)receiver_tmp;
                    MPI_Get(receiver_tmp2, 1, receiver_subarray_type[j], requesterid_table[j], mod_number * datasize[j], 1, subarray_type[j], win_data);
                    MPI_Win_flush(requesterid_table[j], win_data);
                }
            }
            tmp = count_worker;
            MPI_Put(&tmp, 1, MPI_INT, head_requester, 0, 1, MPI_INT, win_count_worker);
            //return 1 means the worker has got data from requester's buffer in one turn
            return 1;
        }
        else return 0;
    }
...
int CTCAW_get_data_real4(float *receiver_buffer)
{
    if (myrole != ROLE_WRK)
    {
        fprintf(stderr, "%d : CTCAW_get_data_real4() : ERROR : wrong role %d\n", world_myrank, myrole);
        return 0;
    }
    if (count_worker == loop_number - 1)
    {
...
int CTCAW_get_data_real8(double *receiver_buffer)
{
    if (myrole != ROLE_WRK)
    {
        fprintf(stderr, "%d : CTCAW_get_data_real8() : ERROR : wrong role %d\n", world_myrank, myrole);
        return 0;
    }
    if (count_worker == loop_number - 1)
    {
