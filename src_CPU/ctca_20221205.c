...
static int worker_buffer_init(int *recv_location_start, int *recv_location_end, int method)
{
    ...
    */
    ...
    {
        recv_dimension_space[dimensionsize - 1 - i] = recv_location_end[dimensionsize - i] - recv_location_start[dimensionsize - i];
    }
    ...
    for (j = 0;j < numrequesters;j++)
    {
        if (calculated[j] > 0)
        {
            for (i = 0;i < dimensionsize;i++)
            {
                ...
                {
                    req_real_start[j * dimensionsize + i] = recv_location_start[i] - location_start[j * dimensionsize + i];
                }
                else 
                {
                ...
                {
                    req_real_end[j * dimensionsize + i] = location_end[j * dimensionsize + i] - location_start[j * dimensionsize + i];
                }
                else 
                {
                    req_real_end[j * dimensionsize + i] = recv_location_end[i] - location_start[j * dimensionsize + i];
                }
    ...
    for (j = 0;j < numrequesters;j++)
    {
        if (calculated[j] > 0)
        {
            wrk_data_size[j * dimensionsize + dimensionsize - 1] = recv_location_end[dimensionsize - 1] - recv_location_start[dimensionsize - 1];
            ...
            {
                wrk_data_size[j * dimensionsize + i] = (recv_location_end[i] - recv_location_start[i]) * wrk_data_size[j * dimensionsize + i + 1];
                req_data_size[j * dimensionsize + i] = (location_end[j * dimensionsize + i] - location_start[j * dimensionsize + i]) * req_data_size[j * dimensionsize + i + 1];
            }
            //req_data_size[j * dimensionsize + dimensionsize - 1] = location_end[j * dimensionsize + dimensionsize - 1] - location_start[j * dimensionsize + dimensionsize - 1];
            
            //req_data_size[j * dimensionsize + dimensionsize - 1] = location_end[j * dimensionsize + dimensionsize - 1] - location_start[j * dimensionsize + dimensionsize - 1];
            wrk_data_disp[j * dimensionsize + dimensionsize - 1] = req_real_start[j * dimensionsize + dimensionsize - 1] + location_start[j * dimensionsize + dimensionsize - 1] - recv_location_start[dimensionsize - 1];
            ...
            for (i = dimensionsize - 2;i >= 0;i--)
            {
                req_data_disp[j * dimensionsize + i] = (location_end[j * dimensionsize + i + 1] - location_start[j * dimensionsize + i + 1]) * req_real_start[j * dimensionsize + i];
                wrk_data_disp[j * dimensionsize + i] = (recv_location_end[i + 1] - recv_location_start[i + 1]) * (req_real_start[j * dimensionsize + i] + location_start[j * dimensionsize + i] - recv_location_start[i]);
...
int CTCAW_buffer_init(int *recv_location_start, int *recv_location_end, int method)
{
    if (myrole != ROLE_WRK)
    {
    ...
    worker_buffer_init(recv_location_start, recv_location_end, method);
}

int requester_buffer_load_data_nosuspend(void *loaded_data)
{
    count_requester++;
    int tmp;
    int i, j;
...
static int requester_buffer_load_data_suspend(void *loaded_data)
{
    /*
    if (subrank_table[world_myrank] == 0)
    {
...
int CTCAR_buffer_load_data(void *loaded_data)
{
    if (myrole != ROLE_REQ)
    {
    ...
    if (buffer_method == 1)
    {
        requester_buffer_load_data_suspend(loaded_data);
    }
    else 
    {
        requester_buffer_load_data_nosuspend(loaded_data);
    }
}
...
static int worker_buffer_read_data_suspend(void *receiver_address)
{
    if (residue > 0)
    {
        residue--;
    ...
    if (subrank_table[world_myrank] == 0)
    {
        ...
        if (mod_flag == 0)
        {
            for (j = 0;j < numrequesters;j++)
            {
                if (calculated[j] == 1)
                {
                    ...
                    worker_loop_read_data(dimensionsize, j, tmp_count_requester - count_worker, receiver_address, mod_number);
        ...
        else 
        {
            for (j = 0;j < numrequesters;j++)
            {
                if (calculated[j] == 1)
                {
                    worker_loop_read_data(dimensionsize, j, mod_bound - count_worker - 1, receiver_address, mod_number);
                    worker_loop_read_data(dimensionsize, j, tmp_count_requester - mod_bound + 1, receiver_address + datatype_size * receiver_size * (mod_bound - count_worker - 1), 0);
                }
            }
        }
...
int worker_buffer_read_data_nosuspend(void *receiver_address)
{
    if (residue > 0)
    {
        residue--;
        ...
        if (mod_flag == 0)
        {
            for (j = 0;j < numrequesters;j++)
            {
                if (calculated[j] == 1)
                {
                    switch (buffer_type) {
                        case DAT_INT:
                        MPI_Get((int *)receiver_address, tmp_count_requester - count_worker, receiver_subarray_type[j], requesterid_table[j], mod_number * datasize[j], tmp_count_requester - count_worker, subarray_type[j], win_data);
                        break;
                        case DAT_REAL4:
                        MPI_Get((float *)receiver_address, tmp_count_requester - count_worker, receiver_subarray_type[j], requesterid_table[j], mod_number * datasize[j], tmp_count_requester - count_worker, subarray_type[j], win_data);
                        break;
                        case DAT_REAL8:
                        MPI_Get((double *)receiver_address, tmp_count_requester - count_worker, receiver_subarray_type[j], requesterid_table[j], mod_number * datasize[j], tmp_count_requester - count_worker, subarray_type[j], win_data);
                        break;
                    }
        ...
        else 
        {
            for (j = 0;j < numrequesters;j++)
            {
                if (calculated[j] == 1)
                {
                    switch (buffer_type) {
                        case DAT_INT:
                        MPI_Get((int *)receiver_address, mod_bound - count_worker - 1, receiver_subarray_type[j], requesterid_table[j], mod_number * datasize[j], mod_bound - count_worker - 1, subarray_type[j], win_data);
                        break;
                        case DAT_REAL4:
                        MPI_Get((float *)receiver_address, mod_bound - count_worker - 1, receiver_subarray_type[j], requesterid_table[j], mod_number * datasize[j], mod_bound - count_worker - 1, subarray_type[j], win_data);
                        break;
                        case DAT_REAL8:
                        MPI_Get((double *)receiver_address, mod_bound - count_worker - 1, receiver_subarray_type[j], requesterid_table[j], mod_number * datasize[j], mod_bound - count_worker - 1, subarray_type[j], win_data);
                        break;
                    }
                    MPI_Win_flush_all(win_data);
                }
            ...
            for (j = 0;j < numrequesters;j++)
            {
                if (calculated[j] == 1)
                {
                    switch (buffer_type) {
                        case DAT_INT:
                        MPI_Get((int *)receiver_address + receiver_size * (mod_bound - count_worker - 1), tmp_count_requester - mod_bound + 1, receiver_subarray_type[j], requesterid_table[j], 0, tmp_count_requester - mod_bound + 1, subarray_type[j], win_data);
                        break;
                        case DAT_REAL4:
                        MPI_Get((float *)receiver_address + receiver_size * (mod_bound - count_worker - 1), tmp_count_requester - mod_bound + 1, receiver_subarray_type[j], requesterid_table[j], 0, tmp_count_requester - mod_bound + 1, subarray_type[j], win_data);
                        break;
                        case DAT_REAL8:
                        MPI_Get((double *)receiver_address + receiver_size * (mod_bound - count_worker - 1), tmp_count_requester - mod_bound + 1, receiver_subarray_type[j], requesterid_table[j], 0, tmp_count_requester - mod_bound + 1, subarray_type[j], win_data);
                        break;
                    }
                    //MPI_Get(receiver_address + receiver_size * (mod_bound - count_worker - 1), tmp_count_requester - mod_bound + 1, receiver_subarray_type[j], requesterid_table[j], 0, tmp_count_requester - mod_bound + 1, subarray_type[j], win_data);
                    MPI_Win_flush_all(win_data);
                }
            }
        }
        ...
        if (mod_flag == 0)
        {
            for (j = 0;j < numrequesters;j++)
            {
                if (calculated[j] == 1)
                {
                    switch (buffer_type) {
                        case DAT_INT:
                        MPI_Get((int *)receiver_address, count_requester - count_worker, receiver_subarray_type[j], requesterid_table[j], mod_number * datasize[j], count_requester - count_worker, subarray_type[j], win_data);
                        break;
                        case DAT_REAL4:
                        MPI_Get((float *)receiver_address, count_requester - count_worker, receiver_subarray_type[j], requesterid_table[j], mod_number * datasize[j], count_requester - count_worker, subarray_type[j], win_data);
                        break;
                        case DAT_REAL8:
                        MPI_Get((double *)receiver_address, count_requester - count_worker, receiver_subarray_type[j], requesterid_table[j], mod_number * datasize[j], count_requester - count_worker, subarray_type[j], win_data);
                        break;
                    }
                    //MPI_Get(receiver_address, count_requester - count_worker, receiver_subarray_type[j], requesterid_table[j], mod_number * datasize[j], count_requester - count_worker, subarray_type[j], win_data);
                    MPI_Win_flush_all(win_data);
        ...
        else 
        {
            ...
            for (j = 0;j < numrequesters;j++)
            {
                if (calculated[j] == 1)
                {
                    switch (buffer_type) {
                        case DAT_INT:
                        MPI_Get((int *)receiver_address + receiver_size * (mod_bound - count_worker - 1), count_requester - mod_bound + 1, receiver_subarray_type[j], requesterid_table[j], 0, count_requester - mod_bound + 1, subarray_type[j], win_data);
                        break;
                        case DAT_REAL4:
                        MPI_Get((float *)receiver_address + receiver_size * (mod_bound - count_worker - 1), count_requester - mod_bound + 1, receiver_subarray_type[j], requesterid_table[j], 0, count_requester - mod_bound + 1, subarray_type[j], win_data);
                        break;
                        case DAT_REAL8:
                        MPI_Get((double *)receiver_address + receiver_size * (mod_bound - count_worker - 1), count_requester - mod_bound + 1, receiver_subarray_type[j], requesterid_table[j], 0, count_requester - mod_bound + 1, subarray_type[j], win_data);
                        break;
                    }
                    MPI_Win_flush_all(win_data);
                }
...
int CTCAW_buffer_read_data(void *receiver_address)
{
    if (myrole != ROLE_WRK)
    {
    ...
    if (buffer_method == 1)
    {
        return worker_buffer_read_data_suspend(receiver_address);
    }
    else
    {
        return worker_buffer_read_data_nosuspend(receiver_address);
    }
}
...
{
    signal_end = 1;
    for (int i = 0;i < req_numwrkgrps; i++)
    {
        MPI_Put(&signal_end, 1, MPI_INT, req_wrkmaster_table[i], 0, 1, MPI_INT, win_signal_end);
    } 
}
