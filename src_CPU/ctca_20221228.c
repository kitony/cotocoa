...
static int worker_buffer_init(int *grid_number, int *data_size_total)
{
    ...
    */
    ...
    switch (type) {
        case DAT_INT:
        specified_buffersize = (int)((size_t)specified_buffersize * 1024 * 1024 * 1024 / (sizeof(int) * (size_t)datasize));
        break;
    ...
    if (specified_buffersize < buffersize)
    {
        buffersize = specified_buffersize;
    }
    */
    ...
    switch (buffer_type) {
        case DAT_INT:
        datatype_size = sizeof(int);
        data_address = (int *)malloc(buffersize * receiver_size * sizeof(int));
        break;

        case DAT_REAL4:
        datatype_size = sizeof(float);
        data_address = (float *)malloc(buffersize * receiver_size * sizeof(float));
        break;

        case DAT_REAL8:
        datatype_size = sizeof(double);
        data_address = (double *)malloc(buffersize * receiver_size * sizeof(double));
        break;
    }
...
int CTCAW_buffer_init(int *grid_number, int *data_size_total)
{
    if (myrole != ROLE_WRK)
    {
    ...
    worker_buffer_init(grid_number, data_size_total);
}
...
int requester_buffer_load_data_nosuspend()
{
    ...
    memcpy(special_buffer + datasize[subrank_table[world_myrank]] * mod_number * datatype_size, data_address, datasize[subrank_table[world_myrank]] * datatype_size);
