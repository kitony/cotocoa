...
int main()
{
    ...
    */
    ...
    CTCAW_buffer_init_int(100, start, end);
    while(1)
    {
        sum = 0;
        tmp = worker_buffer_get_data_nosuspend(recv[0][0]);
        if (tmp == 1)
        {
            flag = get_overwrite_flag();
            ...
            */
            ...
            if (my_rank == 15)
            {
                printf("timestep is %d, flag is %d, worker%d_outside2:show recv, recv[0][0] is %d, recv[50][50] is %d, recv[149][149] is %d\n", timestep, flag, my_rank, tmp_address[0], tmp_address[0], tmp_address[0]);
            }
