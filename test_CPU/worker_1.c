...
int main()
{
   ...
   datasize[1] = 3000;
    ...
    CTCAW_buffer_init(grid, datasize);
    while(1)
    { 
        tmp = CTCAW_buffer_read_data();
        if (tmp == 1)
        {
            tmp_address = CTCAW_buffer_get_address();
            //do calculation
            //calculation();
            if (my_rank >= 10 && my_rank <= 13)
            {
                printf("worker%d_outside1:%d, %d, %d, %d, %d, %d, %d, %d, %d\n",
                my_rank, tmp_address[0], tmp_address[500], tmp_address[1000], tmp_address[1500], 
                tmp_address[499 * 1500], tmp_address[500 * 1500], tmp_address[999 * 1500], tmp_address[1000 * 1500], tmp_address[1500 * 1500]);
            }
